...
public class GeometryMineralDeposit extends Fragment {
    ...
    String[] depoNewPoints;
    Polygon poly;
    Double lat,lng;
    long repId,mineId;
    private static final List<List<Point>> POINTS = new ArrayList<>();
    private static final List<Point> OUTER_POINTS = new ArrayList<>();
    private static final List<List<Point>> depoPOINTS = new ArrayList<>();
    private static final List<Point> depo_OUTER_POINTS = new ArrayList<>();
    private static final List<com.google.android.gms.maps.model.LatLng> depo_LatLng_List = new ArrayList<>();
    ConstraintLayout mainLayout;
    public GeometryMineralDeposit() {
        // Required empty public constructor
    }

    public static GeometryMineralDeposit newInstance() {
        GeometryMineralDeposit fragment = new GeometryMineralDeposit();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_geometry_mineral_deposit, container, false);

        btnLocation = view.findViewById(R.id.btnLocation);
        btnSave = view.findViewById(R.id.btnSave);
        btnBack = view.findViewById(R.id.btnBack);
        mainLayout = view.findViewById(R.id.mainLayout);
        mapView = view.findViewById(R.id.mineralDepositMapView);
            ...
            public void onClick(View v) {
                if(isInside == true) {
                    ...
                    try {
                        ...
                        jsonObject.put("geometryDepoMadani", "POINT (" + lng + " " + lat + ")");
                    ...
                    public void onStyleLoaded(@NonNull Style style) {
                        ...
                        if(!mineWkt.equals("")) {
                            ...
                            for (int i = 0; i < points.size(); i++) {
                                ...
                                OUTER_POINTS.add(Point.fromLngLat(Double.parseDouble(newPoints[0]), Double.parseDouble(newPoints[1])));
                                POINTS.add(OUTER_POINTS);
                                depo_LatLng_List.add(new com.google.android.gms.maps.model.LatLng(Double.parseDouble(newPoints[1]), Double.parseDouble(newPoints[0])));
                            }
                            poly = Polygon.fromLngLats(POINTS);
                        ...
                        if (mapboxMap != null && mapboxMap.getStyle() != null) {
                            ...
                            handler.postDelayed(() -> {
                                ...
                                if (locationComponent != null && locationComponent.getLastKnownLocation() != null) {
                                    ...
                                    isInside = PolyUtil.containsLocation(new com.google.android.gms.maps.model.LatLng(lat,lng), depo_LatLng_List, false);
                                }
                            }, 2000);
                        }
                    }
            ...
            public void onMapReady(@NonNull MapboxMap mapboxMap) {
                mapboxMap.setStyle(Style.MAPBOX_STREETS, new Style.OnStyleLoaded() {
                    ...
                    public void onStyleLoaded(@NonNull Style style) {
                        ...
                        /////////////////////////////////////////////////////////
                        if(!report.getGeometrydepomadani().equals("")) {
                            ArrayList<String> jebhPoints = new ArrayList();
                            String s, s1, s2, s3, s4;
                            s = report.getGeometrydepomadani();
                            ...
                            depo_OUTER_POINTS.clear();
                            depoPOINTS.clear();
                            for (int i = 0; i < jebhPoints.size(); i++) {
                                depoNewPoints = jebhPoints.get(i).split(",");
                                depo_OUTER_POINTS.add(Point.fromLngLat(Double.parseDouble(depoNewPoints[0]), Double.parseDouble(depoNewPoints[1])));
                                depoPOINTS.add(depo_OUTER_POINTS);
                            }

                            final long depoSourceId, depoLayerId;
                            depoSourceId = repId;
                            depoLayerId = repId;
                            if (style.getSource(String.valueOf(depoSourceId)) != null) {
                                style.removeLayer(String.valueOf(depoLayerId));
                                style.removeSource(String.valueOf(depoSourceId));
                            } else {
                                style.removeLayer(String.valueOf(depoLayerId));
                                style.addSource(new GeoJsonSource(String.valueOf(depoSourceId), Polygon.fromLngLats(depoPOINTS)));
                                style.addLayerBelow(new FillLayer(String.valueOf(depoLayerId), String.valueOf(depoSourceId)).withProperties(
